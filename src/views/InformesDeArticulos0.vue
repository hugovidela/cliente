...
//      let newTree = JSON.parse(JSON.stringify(this.itemsTreeGrupos[0]))
        ...
        let nod = []    // armo el tree ( si hay padres deben ser modificados )
        for (let i=0; i<=this.selection.length-1; i++) {
          if (this.selection[i].children!=undefined) {
            nod.push({
              activo: 1,
              ctt: this.selection[i].children.length,
              id: this.selection[i].id,
              name: this.selection[i].name,
              padre: this.selection[i].padre,
              state: this.selection[i].state
            })
          } else {
            nod.push({
              activo: this.selection[i].activo,
              ctt: this.selection[i].ctt,
              id: this.selection[i].id,
              name: this.selection[i].name,
              padre: this.selection[i].padre,
              state: this.selection[i].state
        ...
        function imprimirArbol(nodo, nivel = 0, doc, coi, sel, rows) {
          ...
          if (s!=-1||nodo.padre==0) {
            ...
            if (artsGrupo.length>0||nodo.children!=undefined) {
              doc.setFontSize(12);
        ...
        imprimirArbol(newTree[0], 0, doc, coi, this.selection, this.rows);
