            ...
            <v-sheet class="pa-4" :color="temas.forms_titulo_bg">
              <v-row>
                ...
                <v-col class="pt-3 pl-10" cols="4" sm="4" md="4">
                  <v-btn v-if="items.length==0 && userId!=1"
                    class="white--text text-capitalize"
                    :color="temas.cen_btns_bg"
                    :dark="temas.cen_btns_dark==true"
                    @click="dialogPlantilla=true">
                    Generar Plantilla
                  </v-btn>
                </v-col>
              </v-row>
        ...
        <v-card v-show="dialogPlantilla" class="fg">
          <v-toolbar flat
            ...
            <v-btn
              ...
              icon @click="dialogPlantilla=false">
              <v-icon>mdi-arrow-left-circle</v-icon>
            </v-btn>
            <span class="headdline">Plantillas de Grupos</span>
            <v-spacer></v-spacer>
            <v-btn
              class="white--text text-capitalize"
              :color="temas.forms_titulo_bg"
          ...
          <v-form ref="form2">
            <v-card-text>
              <v-container>
                <v-row>
                  <v-col cols="12">
                    ¡No tienes Grupos de Artículos definidos para el rubro <b>
                      {{rubro.nombre }}</b>!<br><br>
                    <b>gohu</b> dispone de una serie de Plantillas standards para Rubros.<br>
                    Puedes inicializar tu archivo de Grupos de Artículos en base a ella.<br>
                    Luego podrás adaptarla a tus necesidades.<br><br>
                  </v-col>
                </v-row>
              </v-container>
            </v-card-text>
  ...
  export default {
    ...
    created () {
      ...
      return HTTP().get('/usersrubros').then(({ data }) => {
        ...
        return HTTP().get('rubros').then(({ data }) => {
          this.allRubros = data
...
//      const name = `${item.name} (${item.children.length})`;
        ...
        HTTP().post("/grupos", {nombre: name,rubro_id: this.rubro_id,grupo_id: item.id,activo: 1}).then((res) => {
      ...
      generarPlantilla() {
        ...
        return HTTP().post('generarplantillagrupos', {rubro_id: this.rubro.id }).then((res)=>{
          ...
          if (res=='error') {
            ...
            this.mensaje('¡La plantilla ha sido generada correctamente!', this.temas.cen_card_activo_bg, 1500) 
            this.fetchGrupos(this.rubro)
          }
        })
      },
      ...
      grabar() {
        ...
        if (!this.nuevoRoot) {
          HTTP().patch(`grupos`, {id: this.editado.id,rub: this.rubro_id,nombre: this.editado.nombre,activo: this.editado.activo }).then((res)=>{
            if (res=='error') {
              this.mensaje('¡Opps!, hubo un problema al actualizar el grupo!, consulte con la mesa de ayuda Gohu.', 'red', 3000) 
            } else {
              this.mensaje('El grupo se actualizó correctamente', 'green', 1500) 
              this.fetchGrupos(this.rubro)
            }
          }).catch((err)=>{
            console.log(err);
          ...
          HTTP().post("/grupos", { nombre: this.editado.nombre, rubro_id: this.rubro_id, grupo_id: null, activo: 1 }).then((res) => {
            if (res=='error') {
              this.mensaje('¡Opps!, hubo un problema al actualizar el grupo!, consulte con la mesa de ayuda Gohu.', 'red', 3000)
            } else {
              this.mensaje('¡El grupo se agrego correctamente!', 'green', 1500) 
              this.fetchGrupos(this.rubro)
            }
          })
        }
