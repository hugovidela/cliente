...
export default {
  ...
  methods: {
    ...
    exportExcel: function () {
      ...
      const filename = 'saldosiniciales'
    ...
    buscoSiElTerceroTieneSaldoInicial() {
      ...
      return HTTP().post('/tienesaldoinicial', { tercero: this.tercero_id, cual: this.cual, sucursal: this.sucursal }).then(({ data }) => {
        if (data==true) {
          ...
          return HTTP().post('/tienemovimientos', { tercero: this.tercero_id, sucursal: this.sucursal }).then(({ data }) => {
            if (data[0].ctt!=0) {
