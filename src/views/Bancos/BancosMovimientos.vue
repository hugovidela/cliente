    ...
    <v-flex>
        ...
        <v-col v-for="(ban, idx) in banItems" v-bind:key="idx">
          <div class="fg">
            <v-hover v-slot="{ hover}">
              <v-card tile class="fg my-0 ml-2" max-width="200"
                ...
                <v-card-text class="pt-2 pb-0 pl-4 fg">
                  <v-chip-group column multiple>
                    <v-chip v-for="(cue, i) in ban.cuentas" :key="i"
                      class="ml-3 mt-0" outlined multiple
                ...
                <v-card-actions class="fg">
                  <v-tooltip bottom>
                    ...
                    <span class="fg">Nuevo Movimiento</span>
                  </v-tooltip>
                  <v-tooltip bottom>
                    <template v-slot:activator="{ on }">
                      <v-btn
                    ...
                    <span class="fg">Nuevo Saldo Bancario</span>
                  </v-tooltip>
                  <v-tooltip bottom>
                    <template v-slot:activator="{ on }">
                      <v-btn
                    ...
                    <span class="fg">Imprimir Movimientos</span>
                  </v-tooltip>
                  <v-tooltip bottom>
                    <template v-slot:activator="{ on }">
                      <v-btn
        ...
        <v-col>
          <div>
            <v-card tile class="my-0 ml-2" max-width="230">
              <v-card-text class="fg75 pt-2 pb-0 pl-4">
                <span>
                  <v-row>
                    <v-col cols="5" sm="5" md="5" class="pb-0">Banco</v-col>
                    <v-col cols="7" sm="7" md="7" class="pb-0 text-lg-right">
                      <b>${{ formatoImporte(saldoResumen) }}</b>
                    </v-col>
                  </v-row>
                  <v-row>
                    <v-col cols="6" sm="6" md="6" class="pt-0 pb-0">Empresa</v-col>
                    <v-col cols="6" sm="6" md="6" class="text-lg-right pt-0 pb-0">
                      <b>${{ formatoImporte(saldoEmpresa) }}</b>
                    </v-col>
                  </v-row>
                  <v-row>
                    <v-col cols="6" sm="6" md="6" class="pt-0 pb-0">Ch.Ingr/hoy</v-col>
                    <v-col cols="6" sm="6" md="6" class="text-lg-right pt-0 pb-0">
                  ...
                  <v-row>
                    ...
                    <v-col cols="6" sm="6" md="6" class="text-lg-right pt-0">
      ...
      <v-data-table
          ...
          <v-toolbar flat
            ...
            <v-dialog v-model="dialogSaldoBancario" max-width="400px"
              ...
              <v-toolbar flat
                ...
                <v-spacer></v-spacer>
                <v-btn class="fg85" v-show="saldoBancario.saldo!=0"
                  :color="temas.cen_btns_bg"
                  :dark="temas.cen_btns_dark==true"
        ...
        <template v-slot:item.accion="{ item }">
          <v-tooltip bottom>
            <template v-slot:activator="{ on }">
              <v-btn v-if="(item.abrev=='CHP'|| item.abrev=='DPQ'|| item.abrev=='DPE'||
                           item.abrev=='TRE'|| item.abrev=='TRI')&&item.estado!='A'"
          ...
          <v-menu bottom left v-if="cual=2&&item.estado!='A'">
...
export default {
  ...
  data: () => ({
    ...
    headersValores: [
      ...
      { text: 'Fecha', value: 'fecha', width: 89 },
      { text: 'Tercero', value: 'egresados.tercero.nombre', width: 89 },
      { text: 'Observ', value: 'observ', width: 380 },
      ...
      return HTTP().get('/bancoscodigosdemovimientos').then(({ data }) => {
        ...
        return HTTP().get('/tercerocuentas/'+this.$store.state.tercero).then(({ data }) => {
          ...
          if (this.cueItems.length==0) {
            this.msg.msgTitle = 'Mensaje' 
            this.msg.msgAccion = 'no hay cuentas bancarias'
            let m = '<br>No tienes definidas Cuentas Bancarias.<br><br>'
            m += 'Ve a tu ficha en <b>Clientes</b> y en la solapa '
            m += '<b>Cuentas Bancarias</b> podrás definir tus '
            m += 'Cuentas, Tarjetas y Chequeras.<br>'
            this.msg.msgBody = m
  ...
  methods: {
    ...
    cargoPeriodosDeLaCuenta() {
      return HTTP().get('/bancosperiodos/'+this.cuenta).then(({ data }) => {
        ...
        let perActual = moment().format('YYYY-MM')
        if (this.periodos.findIndex(x=>x===perActual)==-1 && this.periodos.length>0) {
          perActual = this.mes = this.periodos[0].substring(5,7)
        } else {
