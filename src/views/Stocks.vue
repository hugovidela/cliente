    ...
    <v-flex>
      ...
      <v-data-table
                  ...
                  <v-card-text>
                    <v-container>
                      ...
                      <v-row>
                        ...
                        <v-col cols="8" class="pt-4 pl-3">
                          <span><b>A Ajustar</b> Positivo para restar, Negativo para sumar.</span>
                        </v-col>
                      </v-row>
                      <v-row v-show="editarItems>0" class="fg">
                          ...
                          <v-row>
                            ...
                            <v-col cols="5" class="mt-0 pt-0">
                              <v-text-field
                                ...
                                class="mt-0 pt-0" dense disabled
                          ...
                          <div class="pl-0 pr-0">
                            <v-data-table
                              ...
                              <template v-slot:item.activo="{ item }">
                                <v-chip
                                  :color="getColor(item.activo)">{{item.activo?'Sí':'No'}}
                                </v-chip>
                              </template>
                        ...
                        <v-col cols="12" class="pt-0">
                          <v-data-table
                            ...
                            <template v-slot:item.nommar="{ item }">
                              <span>{{item.nommar}}</span>
                            </template>
                            <template v-slot:item.nomgru="{ item }">
                              <span>{{item.nomgru}}</span>
                            </template>
                          </v-data-table>
                        </v-col>
                      </v-row>
                        ...
                        <v-col cols="12" sx="12" mx="12" class="pt-0">
                          <v-data-table
                            ...
                            <template v-slot:item.ajuste="{ item }">
                              <span>{{ formatoImporte(item.ajuste) }}</span>
                            </template>
                          </v-data-table>
                        </v-col>
                      </v-row>
        ...
        <template v-slot:item.observaciones="{ item }">
          <span class="fg">{{ item.observaciones }}</span>
        </template>
        ...
        <template v-slot:item.deposito.nombre="{ item }">
          <span class="fg">{{ item.deposito.nombre }}</span>
        </template>
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      { text: 'FECHA', value:'fecha', align: 'left', width: 94},
...
//    { text: 'U.Medida',   value:'unidadNombre', align: 'left', width: 120},
  ...
  computed: {
    ...
    ...mapState([
      'sucursal', 'sucursales', 'sucursalDemo', 'sucursalFiscal', 'proveedor', 'proveedores', 'tipo',
  ...
  watch: {
    '$store.state.sucursal' () {
      this.comprobantes[0].bg    = this.$store.state.temas.cen_panelcpr_bg
      this.comprobantes[0].dark  = this.$store.state.temas.cen_panelcpr_dark
      this.comprobantes[1].bg    = this.$store.state.temas.cen_panelcpr_bg
      this.comprobantes[1].dark  = this.$store.state.temas.cen_panelcpr_dark
      ...
      return HTTP().get('/afipcomprobantes').then(({ data }) => {
        ...
        return HTTP().get('/marcasbus').then(({ data }) => {
          ...
          return HTTP().get('/grupos').then(({ data }) => {
            ...
            this.headersSelArt[0].text = this.$store.state.codigooid == 'C'?'Código':'ID'
            this.headersSelArt[0].value = this.$store.state.codigooid == 'C'?'codigo':'id'
  ...
  methods: {
    ...mapMutations([
      'alert', 'closeAlert', 'setSucursal', 'setSucursales', 'setCentrales', 'setTema', 'setTemas', 'setNotificaciones',
      'setCaja', 'setVinculasPadres', 'setVinculasHijos', 'setEmpresa', 'setOperario', 'setLevel',
    ]),
...
//    this.acciones.push({nombre: 'Detalles',  icon: 'mdi-glasses'})
      ...
      if (item.estado!='F' && item.estado!='A') {
    ...
    listarHTTP:function(suc) {
      return HTTP().post('/stockindex', {suc: this.sucursal, cpr: this.filtroComprobante }).then(({ data }) => {
    ...
    anularBorrarHTTP:function(itemActual, acc) {
      return HTTP().post('/stockanularborrar', { itemActual: itemActual, accion: acc }).then(({data}) => {
        if (data=='error') {
    ...
    finalizarHTTP(item) {
      return HTTP().post('/stockfinalizar',{itemActual: item}).then(({ data }) => {
        if (data=='ok') {
    ...
    cargarArt(item) {
      ...
      this.articulos = [];
      this.editarItems = 1
      return HTTP().post('/stockcargoitems', { id: this.editado.id, }).then(({ data }) => {
        if (data.length>0) {
          for (let i=0; i<=data.length-1; i++) {
    ...
    buscarArt() {
      ...
      return HTTP().post('/articulosx', {
        ...
        vinculosPadresLic: this.$store.state.vinculosPadresLic,
        vinculosPadresAll: this.$store.state.vinculosPadresAll,
        estricto: this.busArt.indexOf('@')!=-1, codigooid: this.$store.state.codigooid, userex: null, dolar: this.$store.state.dolar, ambiente: 'stocks',
    ...
    plaXls() {
      ...
      return HTTP().post('/stockcargoarticulos', {
        mar: this.marca.id, gru: this.grupo.id, vin: v, pro: this.prov})
        .then(({ data }) => {
          this.itemsXlsAll = []
          if (data.length>0) {
            for (let i=0; i<=data.length-1; i++) {
