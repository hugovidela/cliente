  ...
  <v-layout align-start class="fg">
    <v-flex>
      <v-toolbar flat
        ...
        <span>Administración de Caracteristícas</span>
      </v-toolbar>
              ...
              <v-row>
                <v-col cols="8" sm="8" md="8">
                  <v-text-field
                    ...
                    :color="temas.forms_titulo_bg"
                    :dark="temas.forms_titulo_dark==true"
                    label="Ingrese la característica a buscar"
                    flat solo-inverted hide-details clearable
                    clear-icon="mdi-close-circle-outline">
                  </v-text-field>
                </v-col>
                <v-col class="pt-3 pl-10" cols="4" sm="4" md="4">
                  <v-btn
                    :color="temas.cen_btns_bg"
                    :dark="temas.cen_btns_dark==true"
                    class="white--text text-capitalize"
                    @click="addFather">
                    Nueva Caracteristica Principal
                  </v-btn>
                </v-col>
              </v-row>
            </v-sheet>
  ...
  export default {
    ...
    methods: {
      ...
      addChild(item) {
        ...
        HTTP().post("/create", {nombre: name,caracteristica_id: item.id,activo: 1}).then((res) => {
          ...
          this.$set(item.children[item.children.length-1], "padre",  res.data.caracteristica_id);
          ...
          this.editado.padre  = res.data.caracteristica_id
      ...
      actDesactChild(item) {
        ...
        this.strIdCar = this.strDeCar(armoObjeto)
        this.strIdCar = this.strIdCar.substring(0,this.strIdCar.length-1).split(',')
        if (item.activo) {
          if (articulosAsignados>0) {
            this.msg.msgTitle = '¡Error!'
            m += 'No se puede desactivar esta característica.<br>'
            ...
            this.msg.msgTitle = '¡Desactivar Característica!'
            m += '¿Confirma desactivar la característica?<br>'
            m += 'Todos las características hijas también serán desactivadas!'
            this.msg.msgBody = m
            this.msg.msgVisible = true
            this.msg.msgAccion = 'desactivar caracteristica'
            this.msg.msgButtons = ['Aceptar','Cancelar']
          }
        } else {
          this.msg.msgTitle = 'Activar Característica!'
          this.msg.msgBody = '¿Confirmar activar esta Característica?'
          this.msg.msgVisible = true
          this.msg.msgAccion = 'activar caracteristica'
          this.msg.msgButtons = ['Aceptar', 'Cancelar']
        }
      },
      ...
      delChild(item) {
        ...
        if (cttArtAsignados>0) {
          this.msg.msgTitle = '¡Error!'
          m += 'No se puede borrar esta Característica.<br>'
          m += 'El o alguno de sus hijos tienen artículos asignados!'
          this.msg.msgBody = m
          this.msg.msgVisible = true
          this.msg.msgAccion = 'aceptar'
          ...
          this.msg.msgTitle = '¡Borrar Característica!'
          m += '¿Confirma borrar la Característica?<br>'
          m += 'Todos las Característica dependientes también serán borrados!'
          this.msg.msgBody = m
          this.msg.msgVisible = true
          this.msg.msgAccion = 'borrar caracteristica'
          this.msg.msgButtons = ['Aceptar','Cancelar']
        }
      },
      ...
      msgRespuesta(op) {
        if (op=='Aceptar') {
          if (this.msg.msgAccion=='activar caracteristica'||this.msg.msgAccion=='desactivar caracteristica'||this.msg.msgAccion=='borrar caracteristica') {
            debugger
            return HTTP().post("/caracteristicaschanges", {
              ids: this.strIdCar,
              accion: (this.msg.msgAccion=='activar caracteristica' || this.msg.msgAccion=='desactivar caracteristica') ? 'activardesactivar' : 'borrar',
      ...
      grabar() {
        ...
        if (!this.nuevoRoot) {
          HTTP().patch('/update', {id: this.editado.id, nombre: this.editado.nombre,activo: this.editado.activo }).then((res)=>{
            if (res=='error') {
              this.mensaje('¡Opps!, hubo un problema al actualizar la característica!, consulte con la mesa de ayuda Gohu.', 'red', 3000) 
            } else {
              this.mensaje('La característica se actualizó correctamente', 'green', 1500) 
              this.fetchCar()
            }
          }).catch((err)=>{
            console.log(err);
          ...
          debugger
          HTTP().post("/create", { nombre: this.editado.nombre, caracteristica_id: null, activo: 1 }).then((res) => {
            debugger
            if (res=='error') {
              this.mensaje('¡Opps!, hubo un problema al actualizar la característica!, consulte con la mesa de ayuda Gohu.', 'red', 3000)
            } else {
              this.mensaje('¡La característica se agregó correctamente!', 'green', 1500) 
              this.fetchCar()
            }
          })
        }
        ...
        return HTTP().get('/caracteristicas').then(response => {
          ...
          for(var i in response.data) {
            aux.push({
              ...
              padre: Number(response.data[i].caracteristica_id),
