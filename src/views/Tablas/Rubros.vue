...
export default {
  ...
  data: () => ({
    ...
    items: [],
    headers: [
      { text: 'ID', value: 'id', width: 90, align: 'left', sortable: false },
      { text: 'NOMBRE', value:'nombre', width: 150},
      { text: 'ACTIVO', value:'activo', width: 90},
      { text: 'ACCIONES', value: 'accion', width: 90, sortable: false },
    ],
    editedIndex: -1,
    editado: { id: '', nombre: '', activo: true },
    defaultItem: { id: '', nombre: '', activo: true },
  }),
  computed: {
      ...
      return HTTP().post('/'+this.modelo, {
        ...
        user_id: this.userId
          ...
          if (data == 'error') {
            ...
            this.mensaje('¡Rubro creado con éxito!', this.temas.forms_titulo_bg, 1500) 
          }
          this.listarHTTP();
        });
    },
      ...
      return HTTP().patch(`${this.modelo}/${data.id}`, data)
        .then((data) => {
          if (data=='error') {
            ...
            this.mensaje('¡Actualización existos!', this.temas.forms_titulo_bg, 1500) 
          }
          this.listarHTTP();
        });       
    },
